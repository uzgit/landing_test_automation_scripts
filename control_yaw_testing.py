...
def yaw_displacement_callback(data):
    
    global yaw_displacement
    yaw_displacement = data.data
...
column_names = ['time', 'iris_position_x', 'iris_position_y', 'iris_position_z', 'iris_linear_velocity_x', 'iris_linear_velocity_y', 'iris_linear_velocity_z', 'landing_pad_position_x', 'landing_pad_position_y', 'landing_pad_position_z', 'landing_pad_linear_velocity_x', 'landing_pad_linear_velocity_y', 'landing_pad_linear_velocity_z', 'landing_phase', 'plane_displacement', 'energy_consumed', 'latitude', 'longitude', 'altitude', 'yaw_displacement']
...
rospy.Subscriber("/landing_pad/yaw_displacement", Float64, yaw_displacement_callback)
...
def run():
    ...
    if( landing_pad_position_x >= 40):
        set_landing_pad_velocity(-5)

    while( landing_pad_position_x >= 30.5 ):
        time.sleep(0.1)
    ...
    # get the landing pad moving
    time.sleep(2)
    set_landing_pad_velocity(2)
    ...
    dataframe.to_csv("/home/joshua/Documents/moving_land_testing/test_%s.csv" % (i))
...
def yaw_test(east_position, north_position, up_position, theta):
    ...
    # with zero ENU velocities
    enable_landing()
    dataframe = pandas.DataFrame(columns = column_names)

    start_time = rospy.Time.now()
    ten_seconds = rospy.Duration(10)
    loop_rate = rospy.Rate(20)
    while( yaw_displacement > 0.02 or (rospy.Time.now() - start_time <= ten_seconds)):
        # save a single row of data
        values = [rospy.get_time(), iris_position_x, iris_position_y, iris_position_z, iris_linear_velocity_x, iris_linear_velocity_y, iris_linear_velocity_z, landing_pad_position_x, landing_pad_position_y, landing_pad_position_z, landing_pad_linear_velocity_x, landing_pad_linear_velocity_y, landing_pad_linear_velocity_z, landing_phase, plane_displacement, energy_consumed, latitude, longitude, altitude, yaw_displacement]
        new_row = dict(zip(column_names, values))
        dataframe = dataframe.append(new_row, ignore_index=True)
    ...
    dataframe.to_csv("/home/joshua/Documents/yaw_pid_testing/test_%s.csv" % (theta))
...
goto_wait(east_position, north_position, up_position, 1.6)

for theta in thetas:
    yaw_test(east_position, north_position, up_position, theta)
